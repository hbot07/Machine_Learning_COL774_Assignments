...
def train_and_evaluate(C, folds, X_train_shuffled, y_train_shuffled, X_val, y_val):
    ...
    for i, (start, end) in enumerate(folds):
        ...
        #
        # X and Y folds should contain equal elements of each class

        X_train_fold = []
        y_train_fold = []

        for img_class in range(6):
            X_train_fold.append(X_train_shuffled[start:end][y_train_shuffled[start:end] == img_class][:5])
            y_train_fold.append(y_train_shuffled[start:end][y_train_shuffled[start:end] == img_class][:5])

        # X_train_fold = np.concatenate(X_train_fold)
        # y_train_fold = np.concatenate(y_train_fold)

        print(list(y_train_fold))


        # clf = multiclass_svm(X_train_fold, y_train_fold, C)
        # accuracies.append(clf.score(X_val_fold, y_val_fold))

    cv_accuracy = 0#np.mean(accuracies)
    clf = multiclass_svm(X_train_shuffled, y_train_shuffled, C)
    val_accuracy = 0#clf.score(X_val, y_val)
...
plt.ylabel("Accuracy")
plt.legend()
plt.savefig("cross_validation_mult real 2.png")
plt.show()
